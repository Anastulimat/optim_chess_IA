...
void sync_board()
{
	int side_start_index[2] = { 2, 18 };
	memset(board, 0, sizeof(board));
	memset(pospiece, PIECE_DEAD, sizeof(pospiece));

	for (int i = 0; i < 64; i++)
	{
		if (color[i] != EMPTY) //On trouve une pièce sur l'échequier
		{
			if (piece[i] == KING) //On trouve un roi
			{
				if (color[i] == LIGHT)
				{
					pospiece[1] = i;
			...
			else //On trouve d'autre pièces que le roi
			{
				pospiece[side_start_index[color[i]]] = i;
				board[i] = side_start_index[color[i]];
				side_start_index[color[i]]++;
			}
		}
	}
}
...
BOOL attack(int sq, int s)
{
	...
	/*
	Dans cette fonction on va parcourir que les pieces blanches si on est les blancs ==> donc parcour des pièces de 1 à 16
	et si on est les noire on va parcouri que les pièces noires ==> donc parcour des pièces de 17 à 32
	*/
	int i, j, n;
